...
class cmb_Meta_Box {
	...
	function show() {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			if ( isset( $field['row_class'] ) ) {
				echo '<tr class="' . esc_attr( $field['row_class'] ) . '">';
			} else {
				echo '<tr>';
			}
			...
			switch ( $field['type'] ) {
				...
				case 'text':
					...
					break;
				case 'text_disabled':
					echo '<input readonly="readonly" style="width: 70%; margin-right: 5px;" onClick="this.select();" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />','<button type="button" id="zero_clipboard', $field['id'],'" class="button button-secondary" data-clipboard-text="', '' !== $meta ? $meta : $field['std'], '">Copy to Clipboard</button><span id="copySuccess', $field['id'],'" style="display:none;font-style:normal;margin-left:10px;background-color:#222;padding:6px;color:white;border-radius:5px;"><strong style="font-weight:bolder">Copied!</strong></span>','<p class="cmb_metabox_description">', $field['desc'], '</p>';
					break;
				case 'text_small':
					echo '<input class="cmb_text_small zero_clipboard" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" /><span class="cmb_metabox_description">', $field['desc'], '</span>';
					break;
				case 'text_medium':
				...
				case 'textarea_small':
					...
					break;
				case 'textarea_code':
					echo '<textarea name="', $field['id'], '" id="', $field['id'], '" cols="60" rows="10" class="cmb_textarea_code">', '' !== $meta ? esc_textarea( $meta ) : $field['std'], '</textarea>','<p class="cmb_metabox_description">', $field['desc'], '</p>';
					break;
				case 'select':
